...
> **Proof-of-concept humanitarian information management platform designed for offline deployment in disaster-affected areas with degraded or absent internet connectivity.**  
> Built in response to the **June 24, 2026 Venezuela earthquake** and the large-scale displacement and infrastructure crisis it generated.
...
| **AI triage** | Local LLM (Llama 3.1 8B via Ollama) classifies Spanish-language field reports into structured JSON: severity, infrastructure status, medical need. |
| **GIS integration** | Triage output writes directly to a SpatiaLite database monitored by QGIS — map updates in near-real time as reports arrive. |
...
**AI triage output** (Ollama → structured JSON):
...
│   └── venezuela-context.md       ← operational context and IM problem statement
├── pipeline/
...
- **Bandwidth throttling**: when internet does exist, it is frequently too slow for cloud GIS or data platform use
- **Data sensitivity**: population displacement data involving vulnerable groups requires strict local data governance
- **Language and geography**: field reports use Venezuelan Spanish slang, regional landmark references, and local administrative vocabulary not reliably handled by generic AI prompts

See [`docs/venezuela-context.md`](docs/venezuela-context.md) for the full operational context and information management problem statement.
...
| AI triage | Ollama + Llama 3.1 8B (Q4_K_M) | Runs on consumer GPU (8GB VRAM); ~3s latency per report |
...
GENTE is a **proof-of-concept** demonstrating the architectural approach for offline humanitarian IM. It is not production software and would require institutional vetting, security review, and field testing before deployment. Current limitations:
...
- QGIS project currently pre-loaded with publicly available Venezuela base layers only; satellite imagery tiles require separate offline download
- No multi-hub data synchronization (planned for v2: physical USB/SSD-based "sneakernet" sync protocol)
...
30 years of experience in data architecture, ETL pipeline design, BI, and enterprise systems integration across Venezuela, the Caribbean, and Latin America. GENTE was conceived and developed in July 2026 as part of an application for humanitarian IM positions with IOM's Venezuela Country Office earthquake response, and as a direct contribution to improving information management capacity in the affected region.
